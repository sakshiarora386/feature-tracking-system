...
const FeatureRequestDetail: React.FC<FeatureRequestDetailProps> = ({ id }) => {
  ...
  const { data: featureRequest, error, isLoading } = useGetFeatureRequestByIdQuery(id);
  const [updateStatus] = useUpdateFeatureRequestStatusMutation();
  ...
  return (
    <div className="p-4 border rounded-md">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <p><strong>Requested By:</strong> {featureRequest.requestedBy}</p>
          <p><strong>Priority:</strong> {featureRequest.priority}</p>
        </div>
        <div>
          <p><strong>Created At:</strong> {new Date(featureRequest.createdAt!).toLocaleDateString()}</p>
          <p><strong>Last Updated:</strong> {new Date(featureRequest.updatedAt!).toLocaleDateString()}</p>
        </div>
      </div>
      <div className="mt-4 flex items-center space-x-4">
        <Select label="Status" value={featureRequest.status || ''} onChange={handleStatusChange}>
